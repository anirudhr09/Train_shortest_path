...
def draw_nodes_and_connect(labels,ans):
    turtle.title("Computer Graphics Project")
    turtle.setup(800, 600)
    turtle.bgcolor("black")
    # Set the font and font color
    font_title = ("Arial", 16, "bold")  # Specify the font type, size, and style
    ...
    turtle.color("white")
    # Write the text
    turtle.write("Chennai Railway Network", align="center", font=font_title)

    #font_ans = ("Arial", 10, "bold")
    turtle.penup()
    turtle.goto(0, 100)  # Adjust the coordinates as needed
    turtle.pendown()
    turtle.color("white")
    # Write the text
    turtle.write(ans, align="center", font=("Arial", 10, "bold"))
...
if __name__ == '__main__':
    ...
    if area.get(des) is None:
        ...
        exit()
    label.append(src+1)
    path.append(area[src])
    dijkstra(graph, src, des, area)
    path.append(area[des])
    label.append(des+1)
    ans=""
    for i in path:
        ans=ans+i+" - "
    draw_nodes_and_connect(label,ans)